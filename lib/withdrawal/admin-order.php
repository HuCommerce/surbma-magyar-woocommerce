...
/**
 * Get the admin edit URL for a WooCommerce order (HPOS-safe).
 *
 * @param int $order_id Order ID.
 * @return string
 */
function cps_hc_gems_withdrawal_get_order_edit_url( $order_id ) {
	...
	switch ( $column ) {
		case 'wd_order':
			...
			if ( $order_id ) {
				...
				$url   = cps_hc_gems_withdrawal_get_order_edit_url( $order_id );
...
function cps_hc_gems_withdrawal_render_actions_metabox( $post ) {
	...
	if ( $order_id ) {
		echo '<p><strong>' . esc_html__( 'Rendelés:', 'surbma-magyar-woocommerce' ) . '</strong> <a href="' . esc_url( cps_hc_gems_withdrawal_get_order_edit_url( $order_id ) ) . '">#' . esc_html( $order_id ) . '</a></p>';
	}
